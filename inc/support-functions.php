...
// social media
if ( ! function_exists( 'aranoz_social' ) ) {
	function aranoz_social( $args = array()  ){
		...
		$url = aranoz_opt('aranoz_social_url');
...
function aranoz_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'aranoz' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'aranoz' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'aranoz' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function aranoz_get_post_views($postID){
    $count_key = 'aranoz_post_views_count';
...
function aranoz_set_post_views($postID) {
    $count_key = 'aranoz_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
...
// blog post categoty 
function aranoz_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'aranoz' ) );
