...
add_action( 'wp_enqueue_scripts', 'aranoz_sl_enqueue_scripts' );
function aranoz_sl_enqueue_scripts() {

	wp_enqueue_script( 'aranoz-post-likes', ARANOZ_DIR_JS_URI . 'post-likes.js', array( 'aranoz-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_aranoz_process_simple_like', 'aranoz_process_simple_like' );
add_action( 'wp_ajax_aranoz_process_simple_like', 'aranoz_process_simple_like' );
function aranoz_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'aranoz' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'aranoz' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'aranoz' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'aranoz' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'aranoz' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'aranoz' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'aranoz_show_user_likes' );
add_action( 'edit_user_profile', 'aranoz_show_user_likes' );
function aranoz_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'aranoz' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'aranoz' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
